...
function mapProfesional(row: any): Profesional {
  return {
    ...
    config_turnos: row.config_turnos ?? null,  // ← AGREGAR ESTA LÍNEA
  };
}
  ...
  async function getConfiguracionGeneral(_userRole: UserRole): Promise<ConfiguracionGeneral> {
    ...
    // config_turnos existe en la DB pero no en el tipo Profesional
    // lo leemos directamente del row crudo
  ...
  async function updateFolder(folder: Folder): Promise<Folder> {
    const { data, error } = await supabase.from('carpetas_quirurgicas').upsert({
      id_carpeta: folder.id,
  ...
  // ─── GESTIÓN INDIVIDUAL DE PROFESIONALES ─────────────────────────────────────

async function upsertProfesional(
  data: {
    email: string; nombres: string; apellido: string; rol: UserRole;
    especialidad: string; matricula: string; telefono: string;
    activo: boolean; config_turnos: object;
  },
...
async function updateProfesionalConfig(
  email: string,
  data: {
    nombres?: string; apellido?: string; rol?: UserRole;
    especialidad?: string; matricula?: string; telefono?: string;
    activo?: boolean; config_turnos?: object;
  },
  userRole: UserRole = UserRole.ADMINISTRATIVO
...
// Crea usuario en Auth + lo inserta en profesionales
// Requiere que el admin llame esto, luego el nuevo usuario
// recibe un email para setear su contraseña
async function inviteUsuario(
  ...
  // 1. Insertar en profesionales (para que el sistema lo reconozca)
